...
def get_start_time() -> str:
    ...
    L'heure retournée est comprise entre '19h00' et '21h38', le séparateur peut-être 'h', ':'
    ou être absent.
    """
    start_hour = random.randint(19, 21)
    start_minutes = random.randint(0, 59 if start_hour < 21 else 38)
    sep = '' if random.randint(0, 9) % 2 else ('h' if random.randint(0, 9) % 2 else ':')

    value = f"{start_hour:02}{sep}{start_minutes:02}"
